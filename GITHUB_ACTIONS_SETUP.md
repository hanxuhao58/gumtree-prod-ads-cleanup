...
- **自动运行**: 每天凌晨 3:00 北京时间（19:00 UTC 前一天）
- **手动触发**: 访问 Actions 页面，点击 "Run workflow"
  - 可选择运行环境：prod（生产）或 bixi（测试）
  - 可选择是否发送邮件报告
...
on:
  schedule:
    # 修改这里的 cron 表达式
    - cron: '0 19 * * *'  # 每天 19:00 UTC (北京时间凌晨 3:00)
```

**常用时间示例**：
```
'0 19 * * *'  # 每天 19:00 UTC (北京时间凌晨 3:00) - 当前设置
'0 18 * * *'  # 每天 18:00 UTC (北京时间凌晨 2:00)
'0 14 * * *'  # 每天 14:00 UTC (北京时间晚上 10:00)
'0 2 * * *'   # 每天 2:00 UTC (北京时间上午 10:00)
'0 19 * * 1'  # 每周一 19:00 UTC (北京时间凌晨 3:00)
'0 19 1 * *'  # 每月 1 号 19:00 UTC (北京时间凌晨 3:00)
```

**时区说明**：
- GitHub Actions 使用 UTC 时间
- 北京时间 = UTC + 8 小时
- 北京时间凌晨 3:00 = UTC 前一天 19:00
